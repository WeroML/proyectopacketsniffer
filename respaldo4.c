...
WINDOW *raw_window;           //Ventana de info. RAW del paquete selccionado
int current_line = 1;
int scanning = 0;
...
int conteoPackets=0; //Contabilizar cuantos packets detectados van
Packet *packets;
int packetLeer=0;
...
void addPacket(Packet **head, char *id, char *sourceIP, char *destIP, char *protocol, char *sourcePort, char *destPort) {
    // Crear un nuevo nodo
    ...
    strcpy(newNode->sourceIP, sourceIP);
    strcpy(newNode->destIP, destIP);
    strcpy(newNode->protocol, protocol);
    strcpy(newNode->sourcePort, sourcePort);
    strcpy(newNode->destPort, destPort);
...
int main(int argc, char const *argv[]) {
    ...
    raw_window=newwin(WINDOW_HEIGHT,WINDOW_WIDTH/2,start_y+10,start_x+60);
    if (!raw_window) {
        delwin(main_window);
        endwin();
    ...
    while (1) {
        ...
        wattron(main_window, COLOR_PAIR(3));
        mvwprintw(main_window, 7, 2, "Presione P para seleccionar un paquete");
        wattroff(main_window, COLOR_PAIR(3));  
        ...
        if (ch == 'q' || ch == 'Q') {
            ...
            while (current != NULL) {
                if (current->idUnicoPacket == selected_packet) {
                    // Mostrar detalles del paquete en la ventana RAW
                    wclear(raw_window);
                    box(raw_window, 0, 0);
                    mvwprintw(raw_window, 1, 2, "Detalles del Paquete %d:", selected_packet);
                    mvwprintw(raw_window, 3, 2, "ID: %s", current->id);
                    mvwprintw(raw_window, 4, 2, "IP Origen: %s", current->sourceIP);
                    mvwprintw(raw_window, 5, 2, "IP Destino: %s", current->destIP);
                    mvwprintw(raw_window, 6, 2, "Protocolo: %s", current->protocol);
                    mvwprintw(raw_window, 7, 2, "Puerto Origen: %s", current->sourcePort);
                    mvwprintw(raw_window, 8, 2, "Puerto Destino: %s", current->destPort);
